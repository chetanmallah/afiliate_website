...
//                     <p className="text-muted-foreground">Bhandup, Mumbai, Maharashtra, India</p>
//                   </div>
//                 </div>
...
//                     <p className="text-muted-foreground">+91 9892590355</p>
//                   </div>
//                 </div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          access_key: "e69753e0-4956-44ea-b2f3-3ee4966b8421", // <-- Replace this manually
          from_name: `${formData.firstName} ${formData.lastName}`,
          email: formData.email,
          subject: "New Contact Message from Website",
          ...
          <div className={`bg-background/80 backdrop-blur-sm p-6 md:p-8 rounded-2xl border border-border/50 shadow-xl transition-all duration-1000 ${isIntersecting ? 'animate-slide-up' : 'opacity-0'}`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div>
                  <Label htmlFor="firstName" className="text-muted-foreground font-medium">First Name *</Label>
                  <Input id="firstName" name="firstName" value={formData.firstName} onChange={handleChange} placeholder="John" className="mt-2 bg-secondary/50 border-border/50 focus:border-primary" />
                </div>
                <div>
                  <Label htmlFor="lastName" className="text-muted-foreground font-medium">Last Name *</Label>
                  <Input id="lastName" name="lastName" value={formData.lastName} onChange={handleChange} placeholder="Doe" className="mt-2 bg-secondary/50 border-border/50 focus:border-primary" />
                </div>
              </div>
              <div>
                <Label htmlFor="email" className="text-muted-foreground font-medium">Email *</Label>
                <Input id="email" name="email" type="email" value={formData.email} onChange={handleChange} placeholder="john@example.com" className="mt-2 bg-secondary/50 border-border/50 focus:border-primary" />
              </div>
              <div>
                <Label htmlFor="company" className="text-muted-foreground font-medium">Company</Label>
                <Input id="company" name="company" value={formData.company} onChange={handleChange} placeholder="Your Company" className="mt-2 bg-secondary/50 border-border/50 focus:border-primary" />
              </div>
              <div>
                <Label htmlFor="message" className="text-muted-foreground font-medium">Message *</Label>
                <Textarea id="message" name="message" value={formData.message} onChange={handleChange} placeholder="Tell us about your project..." rows={4} className="mt-2 bg-secondary/50 border-border/50 focus:border-primary resize-none" />
              </div>

              <Button
                type="submit"
                className="w-full bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 text-black px-8 py-4 text-lg font-semibold transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl"
                disabled={isSubmitting}
              >
                {isSubmitting ? (
                  <div className="flex items-center gap-2">
            ...
            <div className="bg-background/80 backdrop-blur-sm p-6 md:p-8 rounded-2xl border border-border/50 shadow-xl">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[{ Icon: Linkedin, name: "LinkedIn", color: "hover:bg-blue-600" },
                  { Icon: Twitter, name: "Twitter", color: "hover:bg-blue-400" },
                  { Icon: Instagram, name: "Instagram", color: "hover:bg-pink-600" },
                  { Icon: Facebook, name: "Facebook", color: "hover:bg-blue-700" }].map(({ Icon, name, color }, i) => (
                  <a key={i} href="#" className={`flex items-center gap-3 p-3 bg-secondary/50 hover:bg-primary text-muted-foreground hover:text-white rounded-xl transition-all duration-300 transform hover:scale-105 ${color}`}>
                    <Icon className="h-5 w-5" />
                    <span className="font-medium text-sm">{name}</span>
                  </a>
                ))}
